# UPDATED COLAB SCRIPT WITH TEAM LOGO FETCHING
...
def run_updated_data_fetch_with_logos():
    """Main function to discover leagues and fetch all data including team logos."""
    print("=== STARTING UPDATED DATA FETCH WITH TEAM LOGOS ===")
    ...
    # 3. Fetch supporting data INCLUDING TEAMS WITH LOGOS
    print(f"\n=== FETCHING SUPPORTING DATA WITH TEAM LOGOS ===")
    ...
    # NEW: Fetch teams with logos (this will take longer but includes logos)
    ...
    if conn:
        try:
            ...
            print(f"\n=== COMPLETE DATA FETCH WITH LOGOS FINISHED ===")
...
def quick_verify_updated_data_with_logos():
    """Quick verification of the updated data including team logos in the database."""
    print(f"\n=== QUICK VERIFICATION WITH TEAM LOGOS ===")
    conn = get_db_connection()
    if not conn:
        return
    ...
    try:
        ...
        cur.execute("""
            SELECT 
                ...
                name,
                team_logo_url
            FROM public.teams_new
            WHERE team_logo_url IS NOT NULL
            ORDER BY id
            LIMIT 10;
        """)
        
        sample_teams = cur.fetchall()
        if sample_teams:
            for team in sample_teams:
                print(f"  Team {team[0]} ({team[1]}): {team[2]}")
        else:
            print("  No teams with logos found.")
...
# RUN THE UPDATED DATA FETCH WITH LOGOS
if __name__ == "__main__":
    # Run the updated data fetch with team logos
    run_updated_data_fetch_with_logos()
    
    # Quick verification including logos
    quick_verify_updated_data_with_logos() 