...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processedCountries = countriesData.map((country: any) => ({
      id: country.id,
      name: country.name,
    ...
    const { error } = await supabase
      .from('countries')
      .upsert(processedCountries, { onConflict: 'id' });
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully updated ${processedCountries.length} countries`,
      apiCalls,
      duration,
      timestamp: new Date().toISOString(),
      details: {
        countriesUpdated: processedCountries.length
      }
    });
