...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log('🔄 Starting TV stations update...');
    ...
    // Fetch TV stations from Sportmonks API
    console.log('📺 Fetching TV stations from Sportmonks API...');
    const tvStationsData = await fetchAllPages('https://api.sportmonks.com/v3/football/tv-stations');
    apiCallCount = Math.ceil(tvStationsData.length / 25); // Estimate API calls based on pagination
    
    console.log(`✅ Fetched ${tvStationsData.length} TV stations in ${apiCallCount} API calls`);
    console.log(`Sample TV station data:`, tvStationsData[0]);

    // Process TV stations data to match database schema
    const processedTvStations = tvStationsData.map((tvStation: any) => ({
      id: tvStation.id,
      name: tvStation.name,
      url: tvStation.url,
      image_path: tvStation.image_path
    }));

    console.log(`Processed ${processedTvStations.length} TV stations`);
    console.log(`Sample processed TV station:`, processedTvStations[0]);

    // Populate database with lowercase table name
    await populateTable(supabase, 'tvstations', processedTvStations, ['id', 'name', 'url', 'image_path']);

    const duration = `${((Date.now() - startTime) / 1000).toFixed(2)}s`;

    return NextResponse.json({
      success: true,
      message: `Successfully updated ${processedTvStations.length} TV stations`,
      apiCalls: apiCallCount,
      duration,
      details: {
        tvStationsCount: processedTvStations.length,
    ...
    console.error('TV stations update error:', error);
