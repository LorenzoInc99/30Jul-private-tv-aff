...
export default function TestOddsSimplePage() {
  ...
  useEffect(() => {
    const testOdds = async () => {
      ...
      try {
        ...
        // Check if there are any fixtures with has_odds=true
        const { data: fixturesWithOdds, error: fixturesError } = await supabaseBrowser
          .from('fixtures')
          .select('id, name, has_odds')
          .eq('has_odds', true)
          .limit(5);
                  ...
                  {debugInfo?.allOdds?.data?.length > 0 && (
                    <div>
                      ...
                      <pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-2 overflow-auto">
                        {JSON.stringify(debugInfo.allOdds.data, null, 2)}
                      </pre>
                    </div>
                  )}
                </div>
